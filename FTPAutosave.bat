...
REM ------------------------------------------------------------------------------------------------
SET _DebugStep=CHECK LOG FOLDER& IF !_Debug! EQU TRUE (ECHO.##!_DebugStep!& @ECHO OFF)
REM ------------------------------------------------------------------------------------------------
REM Do not try to use Print Subroutine before defining the date and the log filename and checking if logfolder exists!
IF NOT EXIST "!_LogFolder!" (
	ECHO. -Log folder not found.
	ECHO. --Creating log folder...
	MKDIR "!_LogFolder!"
	IF EXIST "!_LogFolder!" (
		ECHO. ---Log folder created at: [ !_LogFolder! ].
		ECHO.
		PAUSE
...
ECHO.
ECHO.The name of the list will be the name assigned to the
ECHO.completed backup name plus the date/time stamp.
ECHO.
ECHO.Available arguments:
ECHO. [ /? ] to show this help.
ECHO.[ /show ] to pause at the end of program.
ECHO.[ /nolog ] to not include a log in backups.
ECHO.[ /debug ] to display debug steps in program.
ECHO.
